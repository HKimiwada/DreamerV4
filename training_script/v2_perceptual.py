"""
Testing Perceptual Loss (LPIPS: 0.5 weight)
PYTHONPATH=. torchrun --nproc_per_node=1 training_script/v2_perceptual.py
CUDA_VISIBLE_DEVICES=3 PYTHONPATH=. python training_script/v2_perceptual.py
"""
import os
...
class OverfitConfig:
    ...
    lpips_weight = 0.5  # Weight for LPIPS (as in Dreamer4 paper)
    lpips_net = 'alex'  # 'alex', 'vgg', or 'squeeze'

    # Paths
    ckpt_dir = Path("checkpoints/overfit")
    viz_dir = Path("visualizations/overfit_perceptual_loss_v2")
    
    # WandB
    project = "Latest_DreamerV4"
    entity = "hiroki-kimiwada-"
    run_name = "v2_combined_loss_overfit"  # Updated run name
